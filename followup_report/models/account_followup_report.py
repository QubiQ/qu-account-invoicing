...
class AccountFollowupReport(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id=None):
        ...
        line_num = 0
        for l in partner.unreconciled_aml_ids.filtered(lambda l: \
                l.company_id == self.env.user.company_id):
        ...
        for (currency, aml_recs) in res.items():
            ...
            for aml in aml_recs:
                ...
                is_overdue = (today
                              > aml.date_maturity if aml.date_maturity else today
                              > aml.date)
                ...
                lines.append({
                    ...
                    'columns': [type(v) == dict and v or {'name': v}
                                for v in columns],
                    'invoice_number': aml.invoice_id.number,
                    })
            ...
            lines.append({
                ...
                'columns': [{'name': v} for v in ['']
                            * ((3 if self.env.context.get('print_mode'
                            ) else 5)) + [total >= 0 and _('Total Due')
                            or '', total_due]],
                })
            if total_issued > 0:
